...
mod tests {
    ...
    use embedding_database::dao::dao_impl::{get_embedding_user, has_embedding_user, put_embedding_user};
    ...
    async fn test_put() -> anyhow::Result<()> {
       ...
       let put_res = put_embedding_user(&rocksdb, &embedding_user).await?;
         assert_eq!(put_res, ());
        let get_res = get_embedding_user(&rocksdb, embed_id).await?;
        assert_eq!(get_res, Some(embedding_user));

        let is_user = has_embedding_user(rocksdb.clone(), embed_id, &user_uuid)?;
        assert_eq!(is_user, true);

        let not_user = has_embedding_user(rocksdb.clone(), embed_id, &Uuid::new_v4())?;
